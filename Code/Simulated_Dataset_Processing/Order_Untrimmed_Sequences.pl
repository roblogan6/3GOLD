    ...
    if( $file =~ /(.*)(([TAGC]{20})(.*sequences_to_cluster)(\._)(\d+))/ ){
        ...
        while(my $line = <IN>){ 
            if ($line =~ /^>/){ 
                ...
                chomp $line; 
                my ($dist, $rev_dist) = levdist($line, $tag, $ERROR, $FRAMESHIFT); 
                my @distances; 
                push(@distances, $dist, $rev_dist); 
                my $distance = min @distances;
                #if ($distance < 5000){ 
					push@{$hash{$distance}}, $line;
                #}
            }
        }
		foreach my $food (sort {$b <=> $a} (keys %hash)){ 
			if ($food < 6){ 
				push(@output_content, join("\n", @{$hash{$food}}), "\n"); 
				#print $_, "\t", @{$hash{$_}}, "\n"; 
				#print join("\n", @{$hash{$_}}, "\n"); 
			}
		}
        open(OUT, '>', "$ARGV[1]/$output_name.txt") or die $!; 
