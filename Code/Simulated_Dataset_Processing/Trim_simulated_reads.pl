...
# 2nd script in the series. This finds simulated sequences that are within the distance threshold as possible candidates for clustering. 

my $dir = $ARGV[0]; 
my $substitution_weight = 4; 
my $insertion_weight = 2; 
...
foreach my $file (@files){ 
    if ($file =~ /(.*)([TAGC]{20})(.*.txt)/){ 
        ...
        while (my $line = <IN>){ 
            if ($line =~ /(^>.*)([ATGC]{20})/){ 
                my $tag = $2; 
                push(@array_of_tags, $tag); 
                push(@output, $line);
            } else { 
                my $number_of_tags = @array_of_tags; 
                my $index = ($number_of_tags - 1); 
                my $search_tag = $array_of_tags[$index];
                my $windowstart; 
                my $window_size = 20; 
                my $step_size = 1; 
                my $length = length($line); 
                # Use the search tag to search, step size = 1, until there is a perfect match. 
                for ($windowstart = 0; $windowstart <= $length; $windowstart += $step_size){ 
                    my $seq_capture = substr($line, $windowstart, $window_size);
                    # Then change to a 20-mer step size and LD-extraction. 
                    if ($seq_capture eq $search_tag){ 
                        #print "found the match: ", $seq_capture, " at position: ", $windowstart, "\n"; 
                        my $step_length_2 = 20; 
                        for ($windowstart; $windowstart <= $length; $windowstart += $step_length_2){ 
                            my $seq_capture_2 = substr($line, $windowstart, $window_size); 
                            my $error_threshold = 3; 
                            my $frame_shift = 3; 
                            my $weight_threshold = 8; 
                            my $rev_search_tag = reverse $search_tag;
                            my $rev_seq_capture_2 = reverse $seq_capture_2; 
                            my ($dist_1, $dist_2) = levdist($seq_capture_2, $search_tag, $error_threshold, $frame_shift);
				            my ($rev_dist_1, $rev_dist_2) = levdist($rev_search_tag, $rev_seq_capture_2, $error_threshold, $frame_shift);
                            if ((defined $dist_1) && (defined $dist_2) && (defined $rev_dist_1) && (defined $rev_dist_2)){ 
                                if (($dist_1 < ($weight_threshold+1)) || ($dist_2 < ($weight_threshold+1)) || ($rev_dist_1 < ($weight_threshold+1)) || ($rev_dist_2 < ($weight_threshold+1))){
                                    ...
                                    if ($sorted[0] <= $weight_threshold){ 
                                        push(@output, $seq_capture_2, "\n"); 
                                    }
                                }
                            }
                        }
                        my $array_size = @output; 
                        my $almost_size = ($array_size - 1); 
                        my $size = ($almost_size/2); 
                        #print "For the centroid: ", $search_tag, " here are the discovered sequences to cluster:\n"; 
                        open(OUT, '>', "$ARGV[0]/$search_tag._sequences_to_cluster._$size.txt") or die $!; 
                        print OUT @output;  
                        last; 
                    }
                }
