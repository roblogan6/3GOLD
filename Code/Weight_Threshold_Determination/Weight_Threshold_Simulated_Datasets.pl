...
# To be used on simulated datasets
# usage inputs: ./Weight_Threshold_Test_SImulated_Datasets.pl < Multifasta of all simulated sequences for searching > < directory you want to put the output in >

# Will need to manually put in the error profiles for the simulated tool as well as the name of the output file. 
# PaSS = ins 1, del 2, sub 4 
# Nanosim R9 = ins 2, del 1, sub 4
# Both get an error threshold of 3
my $substitution_weight = 4; 
my $insertion_weight = 1; 
my $deletion_weight = 2; 
                    ...
                    if ((defined $dist_1) && (defined $dist_2) && (defined $rev_dist_1) && (defined $rev_dist_2)){ 
                        if (($dist_1 < ($weight_threshold+1)) || ($dist_2 < ($weight_threshold+1)) || ($rev_dist_1 < ($weight_threshold+1)) || ($rev_dist_2 < ($weight_threshold+1))){
                            ...
                            if ($sorted[0] <= $weight_threshold){ 
                                push(@output, $seq_capture_2, "\t", $sorted[0], "\n"); 
                            }
                        }
                    }
                }
...
close IN;
open(OUT, '>', "$ARGV[1]/Collected_Weights_For_Threshold.txt") or die $!; 
print OUT @output; 
